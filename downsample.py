...
for filename in os.listdir(hr_image_dir):
    ...
    cv2.imwrite(os.path.join(lr_image_dir + "/X2", filename[:-4] + "x2.png"), lr_img_2x)
    ...
    if args.keepdims:
        lr_img_3x = cv2.resize(lr_img_3x, hr_img_dims,
                               interpolation=cv2.INTER_CUBIC)
    cv2.imwrite(os.path.join(lr_image_dir + "/X3", filename[:-4] + "x3.png"), lr_img_3x)
    ...
    if args.keepdims:
        lr_img_4x = cv2.resize(lr_img_4x, hr_img_dims,
                               interpolation=cv2.INTER_CUBIC)
    cv2.imwrite(os.path.join(lr_image_dir + "/X4", filename[:-4] + "x4.png"), lr_img_4x)